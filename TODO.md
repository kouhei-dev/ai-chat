...
- [ ] 連続送信の防止（デバウンス/スロットリング）

#### ヘルスチェック
- [ ] /api/health エンドポイントの追加（DB接続確認含む）
- [ ] 起動時の環境変数検証（ANTHROPIC_API_KEY必須チェック）

### 🟡 Medium（堅牢性）

#### データベース最適化
- [ ] Session.expiresAtにインデックス追加（クリーンアップクエリ用）
- [ ] Conversation.sessionIdにインデックス追加
- [ ] Message.roleをEnum型に変更（prisma/schema.prisma:38）
- [ ] onDelete: Cascadeの設定（Conversation→Message）
...
- [ ] MessageListコンポーネントのテスト追加
- [ ] セッション→メッセージ→レスポンスの統合テスト
- [ ] 大きなメッセージ、連続送信のエッジケーステスト

### 🟢 Low（改善・保守性）

#### アクセシビリティ
- [ ] ボタン・入力フィールドにaria-label追加
- [ ] キーボードナビゲーション対応（メッセージ履歴）
